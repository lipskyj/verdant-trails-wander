...
const LightMazeGame: React.FC = () => {
  ...
  const submitRoom3 = () => {
    ...
    setR3Wrong(wrong);
    setAttemptsC((a) => a + 1);
    logEvent('mystery_attempt', { mystery: 'mysteryC', attempt: ++attemptsC.current, correct: wrong.length === 0, detail: { wrong: wrong.length } });
    if (wrong.length === 0) {
      setHasLens(true);
  ...
  const handleSubmit = () => {
    ...
    setWrongIds(wrong);
    setAttemptsA((a) => a + 1);
    logEvent('mystery_attempt', { mystery: 'mysteryA', attempt: ++attemptsA.current, correct: wrong.length === 0, detail: { correct: correct.length, wrong: wrong.length } });
    setChoices((p) => {
      const next = { ...p };
  ...
  const submitChain = () => {
    ...
    setDChainOk(ok);
    setDAttempts((a) => a + 1);
    logEvent('mystery_attempt', { mystery: 'mysteryD', attempt: ++attemptsD.current, correct: ok, detail: { stage: 'chain' } });
    setDFeedback(
      ok
  ...
  const submitFail = () => {
    ...
    setDAttempts((a) => a + 1);
    logEvent('mystery_attempt', { mystery: 'mysteryD', attempt: ++attemptsD.current, correct: ok, detail: { stage: 'apply' } });
  ...
  const submitZoom = () => {
    ...
    logEvent('mystery_attempt', { mystery: 'mysteryE', attempt: ++attemptsE.current, correct: ok });
  ...
  const submitConclusion = () => {
    ...
    logEvent('mystery_attempt', { mystery: 'mysteryB', attempt: ++attemptsB.current, correct: conclusion === 1, detail: { choice: conclusion } });
    if (conclusion === 1) {
      setHasTube(true);
                ...
                {LAB_D.predictOptions.map((opt, i) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      logEvent('mystery_attempt', { mystery: 'mysteryD', attempt: ++attemptsD.current, correct: opt.ok, detail: { stage: 'predict' } });
                      setGameState('room4');
                    }}
