...
export interface BodyPrefs {
  freeMode: boolean; // כל התחנות פתוחות, בקצב ובסדר של החוקר
  withQuizzes: boolean; // שאלות לפני, בתוך ואחרי התחנות
}

interface Props {
  onDone: (prefs: BodyPrefs) => void;
}

const CARD =
  'text-right rounded-2xl border p-4 transition-all backdrop-blur-md hover:-translate-y-0.5 focus:outline-none';

const BodyModeSelect: React.FC<Props> = ({ onDone }) => {
  ...
  const option = (active: boolean, title: string, body: string, onClick: () => void, key: string) => (
    <button
      key={key}
      onClick={onClick}
      className={`${CARD} ${
    ...
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-background/92 backdrop-blur p-4" dir="rtl">
      ...
      >
        <div className="flex flex-col gap-1">
          <span className="text-[11px] tracking-[0.3em] text-accent">{BODY.clubName}</span>
          <h2 className="text-xl font-bold text-primary">איך תרצו לחקור את מסע המזון?</h2>
          <p className="text-xs text-muted-foreground">אפשר לבחור עכשיו — ותמיד אפשר לשנות בהמשך מהמפה.</p>
        </div>

        <div className="flex flex-col gap-2">
          <p className="text-xs font-bold text-foreground">1. סדר וקצב התחנות</p>
          <div className="grid md:grid-cols-2 gap-3">
            {option(
              freeMode === true,
              'חקר חופשי — כל התחנות פתוחות',
              'כל חמש התחנות זמינות מיד. נכנסים לכל ניסוי בכל סדר, בקצב שלכם.',
              () => setFreeMode(true),
              'free'
            )}
            {option(
              freeMode === false,
              'מסלול מודרך — תחנה אחרי תחנה',
              'המסע נפתח לפי סדר הגוף: מהפה ועד המעי הגס, וכל פיצוח מעניק כלי.',
              () => setFreeMode(false),
              'guided'
            )}
          </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {option(
              withQuizzes === true,
              'כן — עם שאלות',
              'שאלון כניסה קצר, בדיקות דופק בתוך התחנות ודו"ח חוקר בסיום.',
              () => setWithQuizzes(true),
              'quiz'
            )}
            {option(
